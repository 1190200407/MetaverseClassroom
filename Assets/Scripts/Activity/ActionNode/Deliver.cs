...
namespace Actions
{
    public class Deliver : BaseActionMethod
    {
        public override void Initialize()
        {
            ...
            {
                EventHandler.Trigger(new NewTaskEvent() { netId = (uint)playerId, taskDescription = "送餐", actionNodeId = actionNode.id });
            }
        }
        ...
        public override IEnumerator ExecuteCoroutine()
        {
            if (playerId == -1)
            {
                Debug.Log($"控制NPC {role} 送餐");
                yield return new WaitForSeconds(1f);
            }
            else
            {
                Debug.Log($"控制玩家 {playerId} 送餐");
